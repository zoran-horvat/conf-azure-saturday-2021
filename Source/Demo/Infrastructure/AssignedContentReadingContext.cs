...
{
    public class AssignedContentReadingContext : TenancyContentContext, IContentReadContext
    {
        public AssignedContentReadingContext(ContentContext context, TenancyProvider tenancyProvider) 
            : base(context, tenancyProvider)
        {
        }

        private IEnumerable<string> GetUserKeys(UserRef authenticatedUser) =>
            new[] { authenticatedUser.Value }.Concat(base.GetReverseFriends().Select(friend => friend.OwnerKey));
