...
namespace Demo.Infrastructure
{
    public abstract class TenancyContentContext
    {
        ...
        public TenancyContentContext(ContentContext context, TenancyProvider tenancyProvider)
        {
            this.DbContext = context;
            this.AuthenticatedUserRef = tenancyProvider.AuthenticatedUser;
        }

        protected abstract Expression<Func<Product, bool>> GetProductsFilter(UserRef authenticatedUser);

        private Lazy<Expression<Func<Product, bool>>> ProductsFilter =>
            new Lazy<Expression<Func<Product, bool>>>(() => this.GetProductsFilter(this.AuthenticatedUserRef));
        ...
        protected IQueryable<FriendConnection> GetFriends() =>
            this.DbContext.QueryAll<FriendConnection>().Where(friend => friend.OwnerKey == this.AuthenticatedUserRef.Value);

        protected IQueryable<FriendConnection> GetReverseFriends() =>
            this.DbContext.QueryAll<FriendConnection>().Where(friend => friend.FriendKey == this.AuthenticatedUserRef.Value);

        protected IQueryable<TResult> GetEntities<TResult>() where TResult : class =>
            typeof(TResult) == typeof(Product) ? this.GetProducts().OfType<TResult>()
            : typeof(TResult) == typeof(FriendConnection) ? this.GetFriends().OfType<TResult>()
        ...
        private TResult FindEntity<TResult>(int id) where TResult : class =>
            typeof(TResult) == typeof(Product) ? this.GetProducts().FirstOrDefault(product => product.Id == id) as TResult
            : typeof(TResult) == typeof(FriendConnection) ? this.GetFriends().FirstOrDefault(friend => friend.Id == id) as TResult
            : default(TResult);
